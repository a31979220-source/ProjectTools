...
* **高颜值 UI & 微动画**：基于精心设计的 Glassmorphic 色彩系统与平滑过度动画，提供媲美顶尖桌面软件的操作体验。

### 📊 2. 多维度项目视图
* **敏捷看板视图 (Kanban Board)**：
  * 支持自定义任务列（待办、进行中、已完成等）、拖拽排序与状态变更。
  * 任务支持优先级（紧急/高/中/低）、便签标签、子任务清单、计划时间段及逾期智能提醒。
* **甘特图时间线 (Gantt View)**：
  * 可视化项目计划与任务排期，支持按日、按周时间跨度缩放，任务时间一目了然。
* **数据统计仪表盘 (Stats Dashboard)**：
  * 直观呈现项目任务完成率、优先级分布、标签热度与工时进度图表。

### 📂 3. 本地目录关联与智能扫描 (Win11 风格)
* **本地工作区关联**：支持一键关联本地代码项目或任意文件目录。
* **三种 Win11 格式视图模式**：
  1. **网格大图标模式 (Grid View)**
  2. **详细信息列表模式 (Details Table View)**
  3. **平铺卡片模式 (Tiles View)**
* **一键生成待办**：支持直接将本地扫描到的文件或目录一键/拖拽转为看板待办任务。

### ⚡ 4. 智能多程序“打开方式”系统 (`OpenWithMenu`)
* **一键快捷唤起**：
  ...
  * 📝 **系统记事本 (Notepad)**。
* **系统原生与外部程序浏览**：
  * ⚙️ **Windows 系统“打开方式”弹窗**：自动唤起 Windows 原生 `OpenWith.exe` 应用选择弹窗。
...
### 4. 打包为 Windows Desktop `.exe` 可执行文件
```bash
npm run pack:exe
```
