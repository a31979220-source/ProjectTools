...
**ProjectTools** 是一款极具现代感、高颜值、流畅且功能强大的**单机版项目管理与敏捷看板桌面应用**。基于 **Electron + React 18 + TypeScript + Tailwind CSS** 打造，专为开发者、项目经理及个人高效工作者设计。应用 100% 本地运行，零云端依赖、无需注册登录，本地数据安全无忧。

---

## 🚀 核心特色与功能亮点

### 1. 极致现代视觉设计 & 正反双向左右平滑滑动动画
* **暗黑 / 浅色模式原生切换**：支持跟随系统或一键平滑切换主题，Windows 标题栏完美融入系统主题颜色 (`nativeTheme`)。
* **高颜值 UI & 视图模式双向滑动动画**：基于精心设计的 Glassmorphic 色彩系统与平滑过渡微动画。在切换【大图标】 (Grid)、【详细信息】 (Details)、【平铺列表】 (Tiles) 模式时，支持基于层级顺序的 **0.55s 缓动与微模糊正反双向左右滑动动画** (Right-to-Left / Left-to-Right Slide)，呈现顶尖桌面软件的操作体验。

### 2. 多维度项目视图与经典看板排版
* **敏捷看板视图 (Kanban Board)**：
  * **4 列经典垂直看板布局**：包含【待办事项】、【进行中】、【审核/测试】、【已完成】4 个状态列，清晰聚焦当前任务流。
  * **卡片拖拽 (Drag & Drop)**：支持在看板列内部自由拖拽排序，以及跨列平滑拖拽移动任务。
  * **卡片直接下拉选择优先级**：卡片与弹窗支持实时点击下拉框快速切换优先级（紧急/高/中/低），即时更新并持久化保存。
  * **各列独立控制显示方式**：每个状态列标题栏均自带【大图标】、【详细信息】、【平铺列表】独立切换按钮，各列风格互不干扰；顶栏提供全局一键同步控制。
  * **全量丰富卡片信息**：卡片完整呈现标题（无截断）、关联物理路径、快捷“打开方式”分拆按钮、优先级、标签、子任务完成进度 Checklist 与截止日期/逾期智能警示。
...
### 4. 本地目录关联与 Win11 风格文件扫描
* **本地工作区关联**：支持一键关联本地物理代码项目或任意文件目录，精简紧凑的垂直边距布局。
* **三种 Win11 格式视图模式**：
  1. **网格大图标模式 (Grid View)**
...
### 7. 智能多源应用自动更新系统 (Auto Update System)
* **GitHub / Gitee Release API 优先感知**：直接对接 GitHub 与 Gitee Latest Release 接口，精准获取最新 Release Tag（如 `v1.1.3`）与 `.exe` 安装包资产，避免被 CDN 静态缓存影响。
...
### 4. 打包为 Windows Desktop 免安装绿化版 (.exe)
```bash
npm run pack:exe
```
